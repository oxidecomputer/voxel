...
/// default**. The image's omicron checkout is NOT configured here: it's derived
/// from the image's commit (`image.cp` -> `<build_root>/omicron-<commit>`) so it
/// can't drift from the image. See `resolve_falcon_env`; `$VOXEL_OMICRON_SRC`
/// overrides.
#[derive(Debug, Clone, PartialEq, Default, Serialize, Deserialize)]
#[serde(default, deny_unknown_fields)]
pub struct Falcon {
...
/// config knob; consumed by [`crate::rss`] (sidecar side) and mirrored by
/// `to_frr` (router side).
#[derive(Debug, Clone, PartialEq)]
