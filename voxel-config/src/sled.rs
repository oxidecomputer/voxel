...
//!
//! Everything that varies per sled is derived from
//! the sled's index and whether it's a scrimlet:
//!
//! - deployment: kind `virtual` on every sled; the SoftNPU device decides
//!   scrimlet vs gimlet at sled-agent startup
//! - deployment.front_port_count: 2 (scrimlet) vs 1
//! - a `# voxel role:` comment that voxel-init reads to gate scadm
...
pub struct SledAgentConfig {
    ...
    /// sled), and the rear-port count + the `tfportrear` list must scale with
...
impl SledAgentConfig {
    ...
    pub fn render(&self) -> String {
        ...
        writeln!(o, "# Sled Agent configuration - generated by voxel (g{i}).")
            .unwrap();
        // Read by voxel-init; sled-agent ignores comments.
        let role = if self.scrimlet { "scrimlet" } else { "gimlet" };
        writeln!(o, "# voxel role: {role}").unwrap();
        writeln!(o).unwrap();
        ...
        // routers). Gimlets have no SoftNPU device, so sled-agent resolves
        // them to plain sleds.
        ...
        writeln!(
            o,
            "deployment = {{ kind = \"virtual\", front_port_count = {front}, rear_port_count = {rear} }}"
        )
        .unwrap();
        ...
        // Emulated M.2 + U.2 vdevs, named per sled. The field shape
...
mod tests {
    ...
    #[test]
    fn scrimlet_fields() {
        let text = old(0, true).render();
        assert!(text.contains("# voxel role: scrimlet\n"));
        let v = parse(&text);
        assert_eq!(v["deployment"]["kind"].as_str(), Some("virtual"));
        assert_eq!(v["deployment"]["front_port_count"].as_integer(), Some(2));
    ...
    fn interconnects_bump_scrimlet_front_ports() {
        ...
        assert_eq!(g["deployment"]["front_port_count"].as_integer(), Some(1));
    }

    #[test]
    fn gimlet_fields() {
        let text = old(1, false).render();
        assert!(text.contains("# voxel role: gimlet\n"));
        let v = parse(&text);
        assert_eq!(v["deployment"]["kind"].as_str(), Some("virtual"));
        assert_eq!(v["deployment"]["front_port_count"].as_integer(), Some(1));
        assert!(
