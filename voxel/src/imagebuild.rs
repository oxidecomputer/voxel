...
//! Image bake: boot a one-node builder, run a step inside it, and capture its
//! disk as a registered falcon base image.
...
/// The builder node name. The deployment name is a parameter so image patch
/// cannot collide with an image build.
const NODE: &str = "vbuild";

pub(crate) struct BakeOpts<'a> {
    /// Base image to boot, or an existing voxel image when re-baking one.
    pub base_image: &'a str,
    /// Agent install role, cp or frr.
    pub role: Option<&'a str>,
    /// An in-guest command to run instead of an agent role. With neither, the
    /// builder only boots, a smoke test of a captured image.
    ...
    pub dataset: &'a str,
    /// falcon deployment name: voxel_build, or voxel_patch for image patch.
    pub deploy: &'a str,
    pub disk_gb: u64,
    pub mem_gb: u64,
    pub cores: u8,
...
pub(crate) struct BuilderNetwork {
    /// Host link for the builder's external NIC. None uses falcon's default.
    pub interface: Option<String>,
    /// A cidr and gateway staged as builder-net for the guest to apply. None
    /// where the segment has DHCP.
    pub static_address: Option<String>,
}

/// Prepare the host side of an image build's network. Isolated mode brings up
/// the voxel segment and stages a static address; lan mode needs nothing.
...
pub(crate) async fn bake(o: BakeOpts<'_>) -> Result<()> {
    ...
    // tearing it down destroys the whole .falcon directory.
    ...
    // illumos guests use mount, linux guests mount_linux. Pick by base image.
    ...
    // An agent role, a command, or neither. The cargo-bay drops the exec bit,
    // so the agent is copied to local disk first.
    ...
    if let Some((label, cmd)) = step {
        ...
        // falcon's exec does not propagate the exit code. Validate the marker
        // content, written only on a complete run.
        let marker = d
...
/// The musl target for the Debian router guest: one static binary.
const FRR_TARGET: &str = "x86_64-unknown-linux-musl";

/// Build a voxel-frr router image: cross-compile the agent for the Debian
/// guest, stage it, bake. Needs voxel's source and a toolchain on the host.
pub(crate) async fn create_frr(
    ...
    // Best effort: an installed target exits nonzero on some rustup versions.
    ...
    // The 9p mount drops the exec bit in the guest. Keep it on the host.
...
/// Resolve a rust toolchain binary: an explicit override, then the rustup
/// install, then PATH. pfexec drops ~/.cargo/bin from PATH.
...
/// voxel's own source tree: next to the binary, else the cwd.
...
/// Stage the builder's static address for an isolated build, or clear a stale
/// one. The cargo-bay is reused, and a leftover breaks a later lan build.
...
/// Clear the device instance map and halt cleanly before capture. A baked map
/// mis-binds vioif instances on deployment nodes; a clean halt flushes it.
async fn quiesce(d: &Runner, node: libfalcon::NodeRef) {
    ...
    let _ = d
        ...
        .await;
    crate::node::wait_for_propolis_exit(NODE, Duration::from_secs(25));
    eprintln!("[voxel] stopping hypervisor");
    crate::node::hyperstop(NODE);
}

/// Capture the builder's disk as a registered falcon base image. Streams into
/// a staging dataset and renames it over the target once the send succeeds.
...
/// A zfs dataset voxel may operate on, validated on construction. An empty or
/// truncated component would widen a recursive destroy to the parent.
...
/// zfs destroy -r, skipped when the target is absent. Presence is checked
/// first, so any failure that surfaces is a real one.
...
/// Stream a snapshot into a new dataset with a full send, so the result has no
/// dependency on the base it was cloned from.
