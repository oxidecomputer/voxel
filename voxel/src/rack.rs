...
// Launch flags plus the config path the power loop service inherits.
    ...
    if no_progress {
        ...
        for rack in 0..racks {
                ...
                topo.rss_sleds().into_iter().find(|(s, _)| s.rack == rack)
            {
                ...
                // Without a staged config-rss (no --init-rss), drive rack setup
                // through the commission API, then watch the bring-up as usual.
                if !init_rss
