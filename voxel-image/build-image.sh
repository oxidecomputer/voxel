...
# --- 4. quiesce + stop --------------------------------------------------------
# Shut the guest down cleanly and wait for propolis to exit before capturing the
# zvol. A fixed short sleep followed by hyperstop can catch Linux images in the
# middle of systemd/Docker shutdown, producing a dirty image that may boot into
# initramfs. Only the Helios control-plane image needs the illumos device-instance
# cleanup; Linux images should just sync and power off.
log "clean guest shutdown before capture"
case "${BASE_IMAGE}" in
debian*|ubuntu*|linux*)
    SHUTDOWN_CMD='sync; sync; (sleep 1; systemctl poweroff --no-wall || poweroff || halt -p) &'
    ;;
*)
    # Clear the baked device-instance map as the LAST thing before capture: a
    # snapshot image must not carry the build VM's NIC/PCI layout, or each
    # deployment node mis-binds vioif instances. An absent map makes each node
    # rebuild it for its own hardware on first boot.
    SHUTDOWN_CMD='pkill -x devfsadmd 2>/dev/null; rm -f /etc/path_to_inst; sync; sync; (sleep 1; halt) &'
    ;;
esac
( cd "${HERE}" && pfexec "${BUILDER}" exec "${NODE}" "${SHUTDOWN_CMD}" ) 2>/dev/null || true

SHUTDOWN_WAIT_SECS="${SHUTDOWN_WAIT_SECS:-60}"
PID_FILE="${HERE}/.falcon/${NODE}.pid"
if [[ -f "${PID_FILE}" ]]; then
    PID="$(cat "${PID_FILE}")"
    log "waiting up to ${SHUTDOWN_WAIT_SECS}s for propolis pid ${PID} to exit..."
    for _ in $(seq 1 "${SHUTDOWN_WAIT_SECS}"); do
        if ! pfexec kill -0 "${PID}" 2>/dev/null; then
            log "guest shutdown complete"
            break
        fi
        sleep 1
    done
    if pfexec kill -0 "${PID}" 2>/dev/null; then
        log "guest shutdown wait elapsed; stopping hypervisor"
        ( cd "${HERE}" && pfexec "${BUILDER}" hyperstop "${NODE}" ) 2>/dev/null || true
    fi
else
    log "WARN: ${PID_FILE} missing; sleeping before hypervisor cleanup"
    sleep "${SHUTDOWN_WAIT_SECS}"
    ( cd "${HERE}" && pfexec "${BUILDER}" hyperstop "${NODE}" ) 2>/dev/null || true
fi
# --- 5. capture --------------------------------------------------------------
...
zfs)
    ...
    if pfexec zfs list -H "${IMG_DS}" >/dev/null 2>&1; then
        if ! pfexec zfs destroy -r "${IMG_DS}"; then
            clones="$(pfexec zfs get -H -o value clones "${IMG_DS}@base" 2>/dev/null || true)"
            log "FATAL: could not replace existing ${IMG_DS}"
            if [[ -n "${clones}" && "${clones}" != "-" ]]; then
                log "FATAL: ${IMG_DS}@base still has clones: ${clones}"
                log "FATAL: destroy dependent topology datasets first, or rebuild with IMAGE_NAME=<new-name>"
            fi
            exit 1
        fi
    fi
