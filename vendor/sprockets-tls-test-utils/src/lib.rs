...
use pki_playground::{
    config::{
        ...
        Certificate, CertificateList, CertificatePoliciesExtension,
        CertificatePolicy, DiceTcbInfoExtension, DigestAlgorithm, Document,
        Entity, EntityNameComponent, Fwid, KeyPair, KeyType, KeyUsageExtension,
    ...
    (
        ...
        Entity {
            ...
            base_dn: vec![
                ...
                EntityNameComponent::OrganizationName(
                    "Oxide Computer Company".into(),
                ),
            ],
        },
        Certificate {
            ...
            extensions: Some(vec![
                X509Extensions::SubjectKeyIdentifier(
                    SubjectKeyIdentifierExtension { critical: false },
                ),
...
pub fn platform_id(n: usize) -> String {
    format!("PDV2:913-0000019:RRR:BRM4422000{}", n + 1)
}
...
mod tests {
    ...
    pub fn sprockets_example_matches_code_based_generation() {
        let path = Utf8PathBuf::from(concat!(
            ...
            "/../test-keys/config.kdl"
        ));

        let loaded = config::load_and_validate(&path)
            .expect("config loads and validates");
